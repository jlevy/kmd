...
from kmd.file_storage.workspaces import current_tmp_dir
...
from kmd.text_formatting.html_in_md import html_img
from kmd.text_formatting.text_formatting import fmt_path
from kmd.util.string_replace import insert_multiple
...
class InsertFrameCaptures(CachedDocAction):
    ...
    def run_item(self, item: Item) -> Item:
        ...
        timestamp_offsets = list(extractor.extract_all())

        log.message(
            f"Found {len(timestamp_offsets)} timestamps in the document, {parse_divs(item.body).size_summary()}."
        )

        # Extract frame captures.
        # TODO: Put these in the store as assets.
        target_dir = current_tmp_dir() / "assets"
        timestamps = [timestamp for timestamp, _offset in timestamp_offsets]
        frame_paths = capture_frames(video_path, timestamps, target_dir)
        ...
        insertions = [
            (
                ...
                html_img(
                    ...
                    class_name="frame-capture",
                ),
            )
            for (timestamp, offset), frame_path in zip(timestamp_offsets, frame_paths)
        ]
