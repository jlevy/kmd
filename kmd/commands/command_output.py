...
def output_heading(message: str, *args, text_wrap: Wrap = Wrap.NONE):
    rprint()
    rprint(Text(fill_text(message % args, text_wrap), COLOR_HEADING))
    rprint()


def format_docstr(name: str, doc: str) -> Text:
    doc = textwrap.dedent(doc).strip()
    wrapped = fill_text(doc, text_wrap=Wrap.HANGING_INDENT)
    return Text.assemble((name, COLOR_KEY), (": ", COLOR_HINT), (wrapped, COLOR_PLAIN))
